...
sIfNode*% sIfNode*::initialize(sIfNode*% self, sNode*% if_exp, sNodeBlock if_block, list<sNode*%>*% elif_exps, list<sNodeBlock>*% elif_blocks, sNodeBlock? else_block)
{
    self.id = gNodeID++;
    self.if_exp = if_exp;
    ...
    self.else_block = clone else_block;
