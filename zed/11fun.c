...
sFunNode*% sFunNode*::initialize(sFunNode*% self, string name, sNode*% obj, list<sNode*%>*% params, sNodeBlock block)
{
    self.id = gNodeID++;
    self.name = name;
    self.obj = obj;
    self.params = params;
    self.block = clone block;
            ...
            switch(obj.kind) {
                ...
                case kListValue: {
                    if(fun_name === "join") {
                        ...
                        if(params.length() == 0) {
                            list<ZVALUE*%>*% list1 = obj.listValue.map_heap {
                                using unsafe;
                                ...
                                ZVALUE*% tmp = clone info2->result_value;
